...
- No bullets, no headers except title
- Flowing paragraphs with tier-specific content
- Sultanic psychological beats in prose
"""
...
def extract_content_from_markdown(markdown_text):
    """Extract key content from markdown for prose rewrite"""
    ...
    # Get intro (first paragraph after title)
...
def write_clean_prose_description(content, tier, weeks):
    """Write description in clean prose format"""
    ...
    # Opening paragraph: tier-specific promise
    desc += f"{content['intro']}\n\n"
    ...
    if content['plan_includes']:
        # Convert first few items to prose
        items = content['plan_includes'][:5]
        includes_text += ' '.join([item.replace('**', '').replace('—', '—') for item in items])
    else:
        includes_text += "Race-specific power zones calibrated from YOUR FTP, with RPE guidance when power isn't available. Technical skills that keep you upright when others crash. Structured taper in final 2 weeks that peaks fitness without losing form."
    ...
    if content['guide_covers']:
        items = content['guide_covers'][:4]
        guide_text += '. '.join([item.replace('**', '').replace('—', '—') for item in items]) + '.'
    else:
        guide_text += "Polarized principles that build both endurance and speed. How base, build, and peak phases stack together for race day. Precise power targets and RPE guidance for structured progression."
    desc += f"{guide_text}\n\n"
    ...
    desc += f"**Real numbers:** {weeks} weeks, {workouts} workouts, "
    if tier == 'ayahuasca':
        desc += "0-5 hours per week. "
    elif tier == 'finisher':
        desc += "8-12 hours per week. "
    elif tier == 'compete':
        desc += "12-18 hours per week. "
    else:
        desc += "18-25+ hours per week. "
    desc += "FTP-based zones with RPE guidance. 18,000+ word guide covering training, technical skills, race execution. $99.\n\n"
    
    # Closing paragraphs
    if content['what_isnt']:
        desc += f"{content['what_isnt']}\n\n"
    else:
        desc += "This isn't a generic training plan with \"Unbound Gravel 200\" stuck on top. It's designed for the specific demands of this race—terrain, distance, conditions. Every workout, every rest day, every taper choice is built around getting you ready for this event.\n\n"
