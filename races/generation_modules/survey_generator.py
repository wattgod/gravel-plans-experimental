...
"""
Survey Generator
Generates plan-specific survey HTML files
"""

import html
from pathlib import Path

SURVEY_TEMPLATE = """<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Training Plan Survey - {race_name} - {plan_name}</title>
...
<body>
    ...
    <div class="plan-info">
        ...
        <div class="plan-info-name">{race_name} - {plan_name}</div>
    </div>
...
def generate_survey_filename(race_name, tier, level):
    """Generate filename for plan-specific survey"""
    # Create URL-friendly slug
    race_slug = race_name.lower().replace(' ', '-').replace('the ', '')
    tier_slug = tier.lower()
    level_slug = level.lower().replace('_', '-').replace(' ', '-')
    
    return f"survey-{race_slug}-{tier_slug}-{level_slug}.html"

def generate_plan_survey(race_data, plan_info, output_dir):
    """Generate a plan-specific survey HTML file"""
    race_name = race_data.get("race_metadata", {}).get("name", "Race")
    tier = plan_info.get("tier", "")
    level = plan_info.get("level", "")
    
    # Format level for display (keep underscores for filename, replace for display)
    level_display = level.replace("_", " ").title()
    if level == "advanced_goat":
        level_display = "Advanced GOAT"
    elif level == "save_my_race":
        level_display = "Save My Race"
    
    # Format plan name
    plan_name = f"{tier.title()} {level_display}"
    
    # Generate survey HTML
    survey_html = SURVEY_TEMPLATE.format(
        race_name=html.escape(race_name),
        plan_name=html.escape(plan_name)
    )
    
    # Create surveys directory
    surveys_dir = Path(output_dir) / "surveys"
    surveys_dir.mkdir(parents=True, exist_ok=True)
    
    # Generate filename
    filename = generate_survey_filename(race_name, tier, level)
    output_path = surveys_dir / filename
